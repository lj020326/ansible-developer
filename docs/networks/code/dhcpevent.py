...
## https://gist.github.com/tomoconnor/931984
...
if (len(sys.argv) > 1):
    ...
    if command == "commit":
        ...
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO records (domain_id,name,type,content,ttl,prio,change_date) VALUES (%s,%s,%s,%s,%s,%s,UNIX_TIMESTAMP(NOW()))",
            [1, hostname, "A", clientIP, 3600, 0])
