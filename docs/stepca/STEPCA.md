...
```shell
root@control01:[docker]$ CONTAINER_ID=$(docker ps --filter "name=docker_stack_step-ca" --format "{{.ID}}")
root@control01:[docker]$ echo "CONTAINER_ID=${CONTAINER_ID}"
CONTAINER_ID=1cca93aa7424
root@control01:[docker]$ docker exec -it ${CONTAINER_ID} sh
~ # which nslookup
/usr/bin/nslookup
~ # nslookup test123.example.int
Server:		127.0.0.11
Address:	127.0.0.11:53


Name:	test123.example.int
Address: 10.0.0.20

~ # ping test123.example.int
PING test123.example.int (10.0.0.20): 56 data bytes
64 bytes from 10.0.0.20: seq=0 ttl=63 time=0.885 ms
64 bytes from 10.0.0.20: seq=1 ttl=63 time=0.246 ms
^C
