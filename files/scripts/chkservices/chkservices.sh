...
CONFIG_FILE="/opt/pyutils/scripts/chkservices/chkservices.${host}.conf"
...
START_SERVICE() {
        ...
        if [ $? -eq 0 ]; then
                ...
                local MAIL_BODY="The service $SERVICE has stopped, and was started successfully by chkservices. Take a look on $HOSTNAME logs, and try to figure out what happened."
                ...
                local MAIL_BODY="The service $SERVICE is stopped and need maintenance to get started. Access $HOSTNAME and check it out!"
...
test -e /etc/systemd/system/$i.service > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
        systemctl status $i 1> /dev/null 2> /dev/null
