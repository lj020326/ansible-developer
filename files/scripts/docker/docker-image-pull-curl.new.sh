...
function main() {
  ...
  while [ $# -gt 0 ]; do
    ...
    response=$(queryManifest "${url}" "${IMAGE_NAME}")
...
function fetch_blob() {
  ...
  local digestBaseUrl="$(determineUrl "${manifestUrl}" "digest")"
  local digestUrl="${digestBaseUrl}/${digest}"

  local header_auth=$(fetchDockerApiAuthHeader "${digestUrl}")
...
function handle_single_manifest_v2() {
  local manifestUrl="${1}"
	...
	fetch_blob "${manifestUrl}" "${image}" "${configDigest}" "${TMP_DIR}/${configFile}" -s
...
#				token="$(curl -fsSL "$authBase/token?service=$authService&scope=repository:$image:pull" | jq --raw-output '.token')"
				...
				fetch_blob "${manifestUrl}" "${image}" "${layerDigest}" "${TMP_DIR}/$layerTar" --progress-bar
				;;
...
function queryManifest() {
  ...
  header_auth=$(fetchDockerApiAuthHeader "${manifestUrl}")
  # If trying to simplify this into a variable "-H $header_auth" you enter quoting hell
  if [[ -z "${header_auth}" ]]; then
      response=$(curl -sL ${DOCKER_HTTP_CURL_HEADERS} "${manifestUrl}")
  else
      response=$(curl -sL -H "${header_auth}" ${DOCKER_HTTP_CURL_HEADERS} "${manifestUrl}")
  fi
...
#	fi
  ...
  header_auth=$(fetchDockerApiAuthHeader "${manifestUrl}")
	...
	if [ "${manifestJson:0:1}" != '{' ]; then
		echo >&2 "error: ${manifestUrl} returned something unexpected:"
		echo >&2 "  $manifestJson"
		exit 1
	fi
			...
			case "$mediaType" in
				*"application/vnd.docker.distribution.manifest.v2+json"* \
				| *"application/vnd.oci.image.manifest.v1+json"* )
					handle_single_manifest_v2 "${manifestUrl}" "${manifestJson}"
					;;
        *"application/vnd.docker.distribution.manifest.list.v2+json"* \
        | *"application/vnd.oci.image.index.v1+json"*)
					...
					for i in "${!layers[@]}"; do
            			...
            			${DOCKER_HTTP_CURL_HEADERS} \
							...
							handle_single_manifest_v2 "${manifestUrl}" "${submanifestJson}"
							found="found"
							break
						fi
					done
		...
		1)
			...
			for i in "${!layers[@]}"; do
				...
				token="$(curl -fsSL "$authBase/token?service=$authService&scope=repository:$image:pull" | jq --raw-output '.token')"
				fetch_blob "${manifestUrl}" "${image}" "${imageLayer}" "${TMP_DIR}/$layerId/layer.tar" --progress-bar
			done
			;;
