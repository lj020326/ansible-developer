...
#
# ref: https://gitlab.com/-/snippets/2483762/raw/main/download-gitlab-frozen-docker-image.sh?inline=false
##################
	...
	for i in "${!layers[@]}"; do
		...
		case "$layerMediaType" in
			application/vnd.docker.image.rootfs.diff.tar.gzip)
				...
				local token
				token="$(curl --user "${CI_REGISTRY_USER}:${CI_REGISTRY_PASSWORD}" -fsSL "$authBase?service=$authService&scope=repository:$image:pull" | jq --raw-output '.token')"
...
#    read -p "Please enter gitlab username: " CI_REGISTRY_USER
#fi
#if [ -z "${CI_REGISTRY_PASSWORD}" ]; then
#    read -s -p "Please enter gitlab password: " CI_REGISTRY_PASSWORD
#    echo ""
#fi
...
while [ $# -gt 0 ]; do
	...
	shift
    ##
    registryBase="https://${imageTag%%/*}"
    authBase="https://${imageTag%%:*}/jwt/auth"
    imageTag="${imageTag#*/}"
    ##
	image="${imageTag%%[:@]*}"
	imageTag="${imageTag#*:}"
...
#	token="$(curl  --user "${CI_REGISTRY_USER}:${CI_REGISTRY_PASSWORD}" -fsSL "$authBase?service=$authService&scope=repository:$image:pull" | jq --raw-output '.token')"
	token="$(curl  -fsSL "$authBase?service=$authService&scope=repository:$image:pull" | jq --raw-output '.token')"
